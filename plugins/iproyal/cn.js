...
/**
 * iproyal-cn 插件
 * 目标：从 IPRoyal API 获取代理列表并仅筛选中国区节点。
 * 
 * 性能优化点：
 * 1. 统一使用单一 proxies 数组管理内存，修复变量名一致性问题。
 * 2. 限制最大抓取页数（50页），防护 1024MB 内存环境下的长队列阻塞。
 */
...
export default async function fetch() {
  ...
  try {
    ...
    const proxies = rawResults.map(item => {
      ...
      if (country !== 'China' && country !== 'CN' && country.toLowerCase() !== 'china') {
        return null;
      }
